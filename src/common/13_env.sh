...
function myenv_create_env_of_user() {
	...
	# 建立基本的配置文件。
	myenv_create_dir ${MYENV_HOME_ENV_PATH}
	myenv_create_env_by_path ${HOME}/.myenv
	mysh_config_create ${MYENV_HOME_ENV_CONFIG_PATH}
	myenv_create_file ${MYENV_HOME_ENV_MAP_NAME_PATH}
}
...
function myenv_create_env_by_path()
{
	...
	mysh_config_create ${TO_DIR}/config.sh
...
{
	local found_path
	found_path=`grep "$1:" $MYENV_HOME_ENV_MAP_NAME_PATH | awk -F ":" '{print $2}'`
	echo $found_path
}
...
function myenv_set_name_of_env_path()
{
	...
	# - 删除同名的
	sed -ri "/^$1:/d" $MYENV_HOME_ENV_MAP_NAME_PATH
	# - 删除同路径的
	local sed_path=$(echo $path | sed 's/\//\\\//g')
	sed -i '/:'${sed_path}'$/d' $MYENV_HOME_ENV_MAP_NAME_PATH
	# - 添加新的
	echo "$1:$path" >> $MYENV_HOME_ENV_MAP_NAME_PATH
...
function mysh_find_env()
{
	...
	myenv_set_cur_path ${MYENV_CUR_PATH}
